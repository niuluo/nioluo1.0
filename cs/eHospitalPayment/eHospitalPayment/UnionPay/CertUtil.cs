...
namespace eHospital.Payment.UnionPay
{
    ...
    public class CertUtil
    {
        ...
        private static void initSignCert(string certPath, string certPwd)
        {
            ...
            WebApiConfig.log.Info("签名证书读取成功，序列号：" + signCert.certId);
        ...
        {
            WebApiConfig.log.Debug("取配置文件证书");
        ...
        private static void initEncryptCert()
        {
            ...
            WebApiConfig.log.Info("加密证书读取成功，序列号：" + encryptCert.certId);
        }
        ...
        private static void initCerCerts()
        {
            ...
            foreach (FileInfo file in files)
            {
                ...
                WebApiConfig.log.Info(file.Name + "读取成功，序列号：" + cert.certId);
            }
        }
        ...
        public static RSACryptoServiceProvider GetValidateProviderFromPath(string certId)// 
        {
            ...
            if (cerCerts == null || cerCerts.Count <= 0)
            {
                WebApiConfig.log.Info("未读取到任何证书……");
                return null;
            }
            ...
            {
                WebApiConfig.log.Info("未匹配到序列号为[" + certId + "]的证书");
                return null;
            }
        }
