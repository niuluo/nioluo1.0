...
namespace eHospital.Payment.UnionPay
{
    public class AcpService
    {
        ...
        public static void Sign(Dictionary<string, string> reqData, Encoding encoding, string certPath, string certPwd)
        {
            ...
            WebApiConfig.log.Info("待签名排序串：[" + stringData + "]");
            ...
            WebApiConfig.log.Info("sha1结果：[" + stringSignDigest + "]");
            ...
            WebApiConfig.log.Info("签名结果：[" + stringSign + "]");
        ...
        public static bool Validate(Dictionary<string, string> rspData, Encoding encoding)
        {
            ...
            WebApiConfig.log.Info("签名原文：[" + signValue + "]");
        ...
        {
            WebApiConfig.log.Info("控件返回报文验签：[" + jsonData + "]");
            //获取签名
        ...
        public static string CreateAutoFormHtml(string reqUrl, Dictionary<string, string> reqData, Encoding encoding)
        {
            ...
            html.AppendFormat("<form name=\"pay_form\" id=\"pay_form\" action=\"{0}\" method=\"post\">", reqUrl);
            ...
            html.AppendLine("document.forms[\"pay_form\"].submit();");
            //    html.AppendLine("}");
            //   html.AppendLine("//-->");
         ...
         //   html.AppendLine("</html>");
            ...
            WebApiConfig.log.Info("生成自动跳转的HTML：[" + result + "]");
            return result;
        }
        ...
        public static Dictionary<String, String> Post(Dictionary<String, String> reqData, String reqUrl, Encoding encoding)
        {
            ...
            try
            {
                ...
                WebApiConfig.log.Info("发送post请求，url=[" + reqUrl + "]，data=[" + postData + "]");
                ...
                {
                    WebApiConfig.log.Info("收到后台应答，data=[" + sResult + "]");
                    return SDKUtil.CoverStringToDictionary(sResult, encoding);
                }
                else
                {
                    string httpStatus = System.Enum.GetName(typeof(HttpStatusCode), webResponse.StatusCode);
                    WebApiConfig.log.Info("非200HTTP状态，httpStatus=" + httpStatus + "，data=[" + sResult + "]");
                    return new Dictionary<string, string>();
                }
            }
            catch (Exception ex)
            {
                WebApiConfig.log.Error("post失败，异常：" + ex.Message);
                return new Dictionary<string, string>();
            }
        ...
        {
            WebApiConfig.log.Debug("发送get请求：" + reqUrl); //get先debug，缴费相关前端接口数据量貌似还挺大
            try
            {
                if ("false".Equals(SDKConfig.IfValidateRemoteCert)) //测试环境不验https证书
                    System.Net.ServicePointManager.ServerCertificateValidationCallback = new RemoteCertificateValidationCallback(ValidateServerCertificate);
                ...
                if (webResponse.StatusCode == HttpStatusCode.OK)
                {
                    WebApiConfig.log.Debug("收到后台应答，data=[" + sResult + "]");
                    return sResult;
                }
                else
                {
                    string httpStatus = System.Enum.GetName(typeof(HttpStatusCode), webResponse.StatusCode);
                    WebApiConfig.log.Debug("非200HTTP状态，httpStatus=" + httpStatus + "，data=[" + sResult + "]");
                    return "";
                }
            }
            catch (Exception ex)
            {
                WebApiConfig.log.Error("get失败，异常：" + ex.Message);
                return ex.Message;
            }
        ...
        public static string EnCodeFileContent(string filePath, Encoding encoding)
        {
            ...
            {
                WebApiConfig.log.Error(filePath + "文件不存在，无法得到fileContent");
                return null;
            }
        }
        ...
        public static bool DeCodeFileContent(Dictionary<string, string> resData, string fileDirectory)
        {
            ...
            {
                WebApiConfig.log.Error("保存fileContent出错：" + e.Message);
                return false;
            }
        }
        ...
        public static Dictionary<string, string> ParseCustomerInfo(string customerInfoStr, Encoding encoding, string certPath, string certPwd)
        {
            ...
            {
                WebApiConfig.log.Error("customerInfoStr解析失败，异常：" + e.Message);
                return new Dictionary<string, string>();
            }
