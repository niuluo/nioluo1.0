...
namespace eHospital.Payment.Alipay
{
    ...
    public class Notify
    {
        ...
        private string _key = string.Empty;                   //商户的私钥
        ...
        public static string getPublicKeyStr(string Path)
        {
            ...
            {
                pubkey = pubkey.Replace("-----BEGIN PUBLIC KEY-----", string.Empty);
                pubkey = pubkey.Replace("-----END PUBLIC KEY-----", string.Empty);
                pubkey = pubkey.Replace("\r", string.Empty);
        ...
        {
            var veryfy_url = Https_veryfy_url + "partner=" + _partner + "&notify_id=" + notify_id;

            //获取远程服务器ATN结果，验证是否是支付宝服务器发来的请求
            var responseTxt = Get_Http(veryfy_url, 120000);
